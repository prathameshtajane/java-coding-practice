...
public class Unique_Morse_Code_Words {
    ...
    public  int uniqueMorseRepresentations(String[] words) {
        ...
        for(String word : words){
            StringBuffer wordInMorse = new StringBuffer();
            for(int i=0; i<word.length() ; i++){
                wordInMorse.append(morse_codes[(int)word.charAt(i)-97]);
            }
            outputSet.add(wordInMorse.toString());
        }
    ...
    public static void main(String args[]){
        ...
        Unique_Morse_Code_Words test  = new Unique_Morse_Code_Words();
        test.uniqueMorseRepresentations(input);
    }
